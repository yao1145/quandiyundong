class AIEnemy {
    constructor(x, y, gameWidth, gameHeight) {
        ...
        this.detectionRadius = Config.PLAYER_DEFAULTS.detectionRadius; // 检测玩家的半径
    ...
    handleBarrierCollision(barrier) {
        ...
        if (deltaX < 0) {
            ...
            this.x = barrier.x - this.size - 2; 
        } else {
            // AI在栏杆右侧，向右反弹
            this.direction = 0; 
            this.x = barrier.x + barrier.width + this.size + 2; 
        }
...
class SurvivalManager {
    ...
    setDifficulty(level) {
        const settings = Config.AI_DEFAULTS;
